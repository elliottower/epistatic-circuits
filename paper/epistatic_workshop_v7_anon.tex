...
Exhaustive sweeps do not scale, and need not: because the spectrum is sparse, LASSO recovers it
from fewer coalitions than there are coefficients ($r > 0.95$ at $M/k = 0.83$ against the exact 15-head spectra, and at $M/k = 0.67$ for a 20-head circuit against a 2{,}000-coalition estimate, exact enumeration there being infeasible). Each discovery method imposes a different selective
pressure on which interactions survive, and the choice of method shapes the circuit one finds.
\end{abstract}
