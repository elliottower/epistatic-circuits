...
\hypersetup{pdftitle={Epistatic Circuits: Exhaustive Interaction Decomposition of Transformer Circuits},
            pdfauthor={Elliot Tower}}

\begin{document}
\maketitle

\begin{abstract}
A circuit is a claim that several components produce a behavior together, yet no
discovery method reports how their effects interact. This is structurally equivalent
to epistasis in genetics: combinatorial perturbations decomposed into individual
effects, pairwise interactions, and higher-order terms. Across five 15-head IOI
circuits under mean ablation, randomly chosen head sets carry 2\% of their coalition
energy at order 2, while circuits selected for functional relevance carry 17--19\% ---
and an additive summary discards all of it. \citet{leemann2024slalom} prove that transformers structurally cannot
represent additive models over input \emph{tokens}; whether additivity fails between a
circuit's \emph{components} is a separate, empirical question, and this paper answers
it. Exhaustive
...
recovers it from fewer coalitions than there are coefficients: $r > 0.95$ at
$M = 100$ for the 15-head circuits ($M/k = 0.83$) and at $M = 140$ for the 20-head
circuit ($M/k = 0.67$, a $7{,}490\times$ reduction over enumeration). Each discovery method imposes a different
...
Restricting the exhaustive lattice to coalitions in which all S-inhibition heads are retained, and separately to those in which all are removed, makes the marginal contribution $\mathbb{E}_S[v(S \cup \{i\}) - v(S)]$ is exact in each condition:
