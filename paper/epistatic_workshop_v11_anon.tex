...
%
% Workshop version, NeurIPS 2026 InterpScience.
% Compile: pdflatex main && bibtex main && pdflatex main && pdflatex main
% =====================================================================
\documentclass{article}

% Two builds come out of this file, differing only in the \usepackage line below.
%   no option  -> anonymous authors, which is what TMLR's portal takes
%   [preprint] -> named authors, which is what Zenodo and arXiv take
% Toggle line 15 itself. A sed for the package name matches these comment lines
% first, which is how both bundles once came out named.
% submissions/tmlr/ holds the anonymous build, submissions/zenodo/ the named one.
...
\begin{abstract}
A circuit is a claim that several components produce a behavior together, yet the methods in common use report component or edge importance without decomposing the circuit's joint effect. \citet{leemann2024slalom} prove that
transformers structurally cannot represent additive models over input tokens; whether additivity
also fails between a circuit's \emph{components} is a separate, empirical question, which we measure here.

We perform exhaustive Walsh--Hadamard coalition sweeps over subcircuits of GPT-2 small, yielding the exact pairwise interaction spectrum --- every deviation from additivity for every pair of attention heads --- for fourteen 15-head circuits spanning indirect object
identification and reverse token identification under three ablation primitives, and for one
20-head circuit. The decomposition is exact and reference-free. Three functionally selected IOI circuits place 17--19\% of their coalition energy at order~2 under mean ablation, against 2\% for a matched random set; the share varies with task and ablation primitive, from about 1\% to 35\% across six tasks. An additive summary discards all of it.

Scored against that ground truth, three families of discovery proxy disagree about what a circuit
is. Weight-space geometry (OV and QK composition scores) carries no cross-validated predictive power for pairwise interaction, with no positive CV~$R^2$ in any condition, while a positive control on the same target and sample size returns CV~$R^2 = 0.20$. Edge Attribution Patching agrees moderately (Spearman $\rho = 0.54$, 95\% CI $[0.37, 0.71]$). Path patching, which isolates direct causal
edges, measures a nearly orthogonal quantity (Spearman $\rho = 0.21$, signed Pearson $r = 0.07$):
functional coupling and information flow are different things, and a circuit selected for one is
not selected for the other.

The spectrum carries mechanism rather than only structure. A complementation test on the
interaction matrix rediscovers that negative name movers invert sign when their upstream
S-inhibition heads are removed --- a dependency the original IOI analysis flagged as unexplained,
and one no additive summary of the same circuit can express.

Exhaustive sweeps do not scale, and do not need to: because the spectrum is sparse, LASSO recovers it
from fewer coalitions than there are coefficients ($r > 0.95$ at $M/k = 0.83$ against the exact 15-head spectra, and at $M/k = 0.67$ for a 20-head circuit against a 2{,}000-coalition estimate, exact enumeration there being infeasible). Each discovery method imposes a different selective pressure on which interactions survive.
\end{abstract}
...
resulting circuit \emph{descriptions} are additive: components are selected by individual scores, and interactions among the selected components are left implicit. This does not make the circuit compute additively. It means the discovery procedure gives no account of the gap between a circuit's joint effect and the sum of its component effects.

Attention violates additivity at the architecture level.
...
question. Any circuit discovery method that
...
of components in a circuit --- data that has not been
...
\begin{enumerate}
\item \textbf{Weight geometry does not predict interaction.} OV composition, QK
  ...
  (Section~\ref{sec:weight-geometry}). The tested pairwise summaries of weight geometry do not predict interaction out of sample; the interaction arises from how the components operate in context under the intervention.

\item \textbf{Different methods recover different structure.} EAP edge scores agree moderately with the Walsh interaction spectrum (Spearman $\rho = 0.54$, 95\% CI $[0.37, 0.71]$). Path patching, which isolates direct causal edges through
...
\item \textbf{The interaction structure carries mechanism.} Applying a
  ...
  when their upstream S-inhibition heads are removed --- a dependency that \citet{wang2022ioi} flagged as unexplained
...
a 20-head circuit from 140 random coalitions ($r > 0.95$ against a 2{,}000-coalition estimate), against the ${\approx}10^6$ coalitions exhaustive enumeration would require
...
Exhaustive sweeps first establish that there is interaction structure worth recovering. Under mean
ablation, three functionally selected 15-head IOI circuits place 17--19\% of their coalition energy
at order~2, against 2\% for a matched random set (Table~\ref{tab:energy}). The share depends on both task
and ablation primitive, running from about 1\% for gendered pronoun circuits to 35\% for a
subject--verb agreement circuit under zero ablation; Appendix~\ref{app:additional} gives the breakdown across
six tasks. Order-3 and higher terms stay under 5\% of the energy in 43 of 48 non-random sweeps, so
the pairwise spectrum is where the structure sits. The rest of this section asks whether that
spectrum can be recovered without enumerating $2^n$ coalitions.
...
\subsection{Sparse Recovery of the Interaction Spectrum}
...
$\Phi_{mc} = (-1)^{\mathrm{popcount}(c \,\&\, s)}$, and solve a sparse regression for the $k = n + \binom{n}{2}$ order-1 and order-2 coefficients. Stopping at order~2 is what the spectra support: order-3 and higher terms stay under 5\% of the energy in 43 of 48 non-random sweeps.
...
linearized gradient flow between heads. Comparing the magnitude-ranked pairwise Walsh coefficients to the corresponding EAP edge scores, computed with the reference implementation of \citet{mueller2025mib} and aggregated over the receiver's query, key and value slots and both directions, gives Spearman $\rho = 0.54$ (delete-one-head jackknife 95\% CI $[0.37, 0.71]$) over the 176 of 190 pairs for which the graph contains an admissible edge. The two methods agree on
...
The three sections that follow use the exact pairwise coefficients from the sweeps above --- not the compressed reconstructions of
...
32\% sign-flip rate has no exact counterpart in the genetics literature.
The closest reported quantity varies the genetic background rather than
the measurement: adding a mutation at a third locus changes the
interaction type of a pair in more than $35\%$ of cases in a functional
background, against below $10\%$ in a non-functional
one~\citep{fluidity2024elife}. Disagreement between loss-of-function
platforms is documented across model organisms without a single rate
attached to it~\citep{housden2017comparison}. Correlating each
...
\paragraph{Projection onto the logit direction.}
...
The interaction is plainly measurable in activation space, and pairwise subspace relationships still fail to predict it. What the coalition
...
EAP edge scores correlate moderately with Walsh interaction coefficients ($\rho = 0.54$; Section~\ref{sec:sparse-walsh-recovery}). Path patching
...
with zero interaction. The two rankings share none of their top-10 entries. Head pairs sharing an endpoint are not independent, so we bound these correlations with the delete-one-head jackknife used for weight geometry above, dropping every pair that touches a head and taking the interval from $t$ on the number of heads rather than the normal quantile. The absolute Spearman correlation is $0.21$ (jackknife SE $0.11$, 95\% CI $[-0.02, 0.44]$) and the signed Pearson correlation is $0.07$ (SE $0.17$, 95\% CI $[-0.29, 0.42]$): neither is distinguishable from zero once the dependence is accounted for. These intervals are a post-hoc robustness analysis; the comparison itself was preregistered. Scored against the same Walsh coefficients over the same pairs and under the same magnitude transformation, EAP reaches $\rho = 0.54$ where path patching reaches $0.21$, and the paired delete-one-head difference is $0.33$ (95\% CI $[0.14, 0.52]$), so the two proxies differ in how much of the interaction spectrum they retain rather than differing only in point estimate.
...
Pairs cluster along both axes independently, with no diagonal structure. The top Walsh pair
...
\subsection{Sign Inversion in Negative Name Movers}
...
Restricting the exhaustive lattice to coalitions in which all S-inhibition heads are retained, and separately to those in which all are removed, makes the marginal contribution $\mathbb{E}_S[v(S \cup \{i\}) - v(S)]$ exact in each condition:
...
Figure~\ref{fig:conditional-marginal} shows both negative heads inverting in every bootstrap draw, moving from clearly harmful to clearly helpful. The name movers decay toward zero: L10H0 keeps its sign, and L9H9 crosses zero at a
...
\subsection{Cost of an Additive Summary}
...
(Table~\ref{tab:energy}); the sign inversion sits in that gap. A head whose contribution depends on the state of another head is the rule rather than the exception wherever order-2 energy exceeds a few percent of the spectrum, and because the energy at each order is measurable, how far a circuit sits past that point is checkable rather than assumed.
...
A head's measured contribution is a
...
spectrum; two methods whose agreement with the spectrum runs from $\rho = 0.54$ (EAP) down to $\rho = 0.21$ (path patching) preserve detectably different structure. The ``right'' discovery method depends on what
...
\citet{miller2024transformer} show that faithfulness metrics are not
...
For a fixed component set, task distribution, outcome metric and ablation primitive, the Walsh
spectrum is the unique orthogonal decomposition of a circuit's coalition response surface, and it
is what the marginal scores discard. Scored against it, the tested weight-geometry summaries recover
none of the interaction structure, EAP recovers some of it, and path patching recovers a quantity
not distinguishable from orthogonal. What they discard is not noise: it carries the S-inhibition
dependency the original IOI analysis reported and left unexplained. Any method that assigns scores
to components can be evaluated the same way.
...
Two regimes bound the required $M$, and the structured one is what applies here. The generic RIP bound for Gaussian or Bernoulli measurement matrices gives
...
Of three recovery algorithms compared, LASSO with cross-validated regularization (LassoCV, 5-fold) selects both which coefficients are nonzero
...
greedily~\citep{tropp2007signal}. Neither method receives oracle
knowledge of $k$. Monte Carlo
...
\citet{chen2025causalheadgating} fit a soft gate per head by gradient
descent on the task metric, recovering a circuit in one optimization run
rather than one ablation per component. The gates are fit jointly, so the
resulting scores are not order-1 quantities in the sense used here; they
carry whatever interaction structure the optimizer exploits, without
separating it into orders.
...
\item \textbf{CPR} (Circuit Performance Recovered): the area under the
  faithfulness curve as a function of circuit size. Higher is better.

\item \textbf{CMD} (Circuit--Model Distance): the area between the
  faithfulness curve and the ideal curve $f = 1$, obtained by integrating
  $|1 - f|$. Lower is better.
\end{itemize}

Both areas are trapezoidal sums over the sampled circuit sizes and are
reported on the scale of the swept interval rather than rescaled.
Neither is bounded: $f$ exceeds 1 whenever an ablated subnetwork produces
a larger logit difference than the full model, and \citet{mueller2025mib}
leave the metrics unbounded for that reason. The two areas are related by
\begin{equation}
  \text{CPR} + \text{CMD} = (x_{\max} - x_{\min}) + 2\int (f - 1)^{+}\,dx,
  \label{eq:cpr-cmd}
\end{equation}
so they sum to the width of the swept interval exactly when $f \le 1$
throughout, and exceed it in proportion to the overshoot. Monotonicity of
$f$ does not enter. Section~\ref{sec:mib-benchmark} reports circuits on
...
\subsection{Variation Across Tasks}
...
GPT-2 small: IOI, repeated token identification
(RTI)~\citep{tower2025rti}, greater-than~\citep{hanna2023greater},
...
\subsection{Faithfulness of Walsh-Ranked Circuits}
...
$m(\varnothing) = 0.905$. CPR and CMD are trapezoidal sums over the 13
sampled sizes, following the reference implementation of
\citet{mueller2025mib}; the swept interval runs from $k/144 = 0.0069$ to
$1$, so an $f \equiv 1$ circuit would score CPR $= 0.993$. The sweep is
over head fraction rather than the edge proportion the MIB leaderboard
uses, and over 13 sizes rather than its 10 percentages, so the values
below rank methods against each other and are not comparable to
leaderboard entries.
...
\midrule
Walsh                              & $0.998$ & $0.121$ \\
Activation patching                & $0.917$ & $0.112$ \\
Walsh + activation patching        & $0.942$ & $0.119$ \\
Random                             & $0.516 \pm 0.260$ & $0.488 \pm 0.243$ \\
\bottomrule
\end{tabular}

\medskip

Walsh-ranked circuits achieve the highest CPR ($0.998$), exceeding
activation patching ($0.917$) by $0.082$. Activation patching has
the lowest CMD ($0.112$ versus $0.121$ for Walsh): its circuits
track the ideal $f = 1$ line more tightly, while Walsh circuits
overshoot it at intermediate sizes. Equation~\ref{eq:cpr-cmd} reads
directly off these curves: the four random rankings that never exceed
$f = 1$ give CPR $+$ CMD $= 0.993$ to four decimal places, the fifth
random ranking gives $1.048$, and Walsh gives $1.120$. The rank-sum
combination does not improve over either component (CPR $= 0.942$,
CMD $= 0.119$),
...
\subsection{EAP--Walsh Agreement: Sensitivity and Aggregation}
\label{app:eap-walsh}

The EAP scores of Section~\ref{sec:sparse-walsh-recovery} come from the reference
implementation of \citet{mueller2025mib} run on stock GPT-2 small over the
\texttt{mib-bench/ioi} dataset (200 examples, logit-difference metric), with
\texttt{EAP-IG-inputs} and \texttt{ig\_steps}~$=5$. A Walsh coefficient for a head pair
is undirected, so the directed edge scores are aggregated by summing $|$score$|$ over the
receiver's query, key and value slots and over both directions. Fourteen of the 190 pairs
have no admissible directed edge in the graph, all of them same-layer, and are excluded.

Plain EAP, which is what \citet{syed2023attribution} describe, gives the same picture:
Spearman $\rho = 0.53$ (95\% CI $[0.33, 0.74]$) against $0.54$ for EAP-IG, so the result
is not specific to the integrated-gradients variant.

The Pearson estimate is close to the Spearman in value and far less stable. For EAP-IG it
is $r = 0.54$ with a delete-one-head jackknife interval of $[-0.49, 0.94]$ on the Fisher
$z$ scale; for plain EAP, $r = 0.57$ with $[-0.42, 0.94]$. Removing a single head moves it
substantially, which is what a linear correlation does when a few high-magnitude pairs
carry most of the leverage. The rank statistic survives the same deletions, so the
agreement we report is an agreement about which pairs interact strongly rather than about
how strongly.


\section{Further Interaction Structure}


\subsection{Double Ablation and Complementation}
...
\subsection{Comparison with the IOI Taxonomy}
...
ablation as well as zero, circuits from EAP-IG~\citep{hanna2024faith} and from order-1 Walsh
...
\subsection{Asymmetry of the Merge}
