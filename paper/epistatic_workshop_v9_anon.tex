...
with zero interaction. The two rankings share none of their top-10 entries. Head pairs sharing an endpoint are not independent, so we bound these correlations with the delete-one-head jackknife used for weight geometry above, dropping every pair that touches a head. The absolute Spearman correlation is $0.21$ (jackknife SE $0.11$, 95\% CI $[-0.00, 0.42]$) and the signed Pearson correlation is $0.07$ (SE $0.17$, 95\% CI $[-0.27, 0.40]$): neither is distinguishable from zero once the dependence is accounted for. This interval is a post-hoc robustness analysis; the comparison itself was preregistered. The corresponding interval for the EAP correlation was not computed, so the gap between $\rho = 0.58$ and $\rho = 0.21$ is a difference in point estimates.
