...
app = modal.App("primitive-geometry-v4")
...
out_vol = modal.Volume.from_name("primitive-geometry-results-v4", create_if_missing=True)
...
def run():
    import json, itertools, time, traceback
    ...
    # Filter to >= 2 arms; flag prompt mismatches instead of skipping
    comparable = {}
    prompt_mismatch_groups = set()
    for (task, hs), arms in sorted(groups.items(), key=lambda x: (-len(x[1]), x[0][0])):
        ...
        if len(prompt_counts) > 1:
            note(f"  PROMPT-MISMATCH {task}/{len(list(hs))}h {sorted(arms.keys())}: "
                 f"prompts {sorted(prompt_counts)} — included with flag")
            prompt_mismatch_groups.add((task, frozenset(hs)))
    ...
    for (task, hs), arms in sorted(comparable.items(), key=lambda x: x[0]):
        ...
        prompts_matched = (task, frozenset(hs)) not in prompt_mismatch_groups
        circuit_key = f"{task}_{n_heads}h_{sorted(list(hs))[:2]}"
        note(f"\n{ts()} Circuit: {task} {n_heads}h ({len(pairs_list)} pairs)"
             f"{'' if prompts_matched else ' [PROMPT MISMATCH]'}")
        ...
        try:
            for abl in ['zero', 'mean', 'resample']:
                if abl not in arms:
                    continue
                entry = arms[abl]
                note(f"    loading {abl}: {entry['filename']} (vkey={entry['vkey']}, prompts={entry['n_prompts']})")
                w_pairs, w_odd, w_even, pairs, n = load_walsh_2d(entry)
                note(f"    WHT done for {abl}: {len(w_pairs)} pairs, n={n}")
                ...
                row = {
                    ...
                    "prompt_matched": prompts_matched,
            ...
            note(f"  ERROR on {task} {n_heads}h: {type(e).__name__}: {e}")
            note(f"  TRACEBACK:\n{traceback.format_exc()}")

        circuit_results[(task, hs)] = arm_results

        Path("/out/primitive_geometry_rows_v4.json").write_text(json.dumps(rows, indent=1))
        out_vol.commit()

    # ---- 5. Save per-pair records for P3 --------------------------------------
    note(f"\n{ts()} Saving {len(pair_records)} per-pair records")
    Path("/out/primitive_geometry_pairs_v4.json").write_text(json.dumps(pair_records, indent=1))
    out_vol.commit()
    ...
    Path("/out/primitive_geometry_summary_v4.json").write_text(json.dumps(summary, indent=2))
    Path("/out/primitive_geometry_rows_v4.json").write_text(json.dumps(rows, indent=1))
    Path("/out/primitive_geometry_pairs_v4.json").write_text(json.dumps(pair_records, indent=1))
    out_vol.commit()
