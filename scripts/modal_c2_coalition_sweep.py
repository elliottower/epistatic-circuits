"""Modal wrapper: C2 coalition sweep (zero + mean ablation).

C2 was discovered by per-head zero-ablation patching on 512 IOI prompts
(prereg v8, SHA f62bd44). This script runs the full 2^15 coalition sweep
for C2 under both zero and mean ablation.

Usage:
    modal run --detach experiments_batch2/genetics/modal_c2_coalition_sweep.py
"""

import modal

app = modal.App("c2-coalition-sweep-zero-mean-512p")

results_volume = modal.Volume.from_name("c2-coalition-sweep", create_if_missing=True)
...
C2_HEADS = [
    (0, 1), (0, 8), (0, 9), (0, 11), (1, 3), (4, 7), (5, 10),
    (6, 9), (7, 9), (8, 3), (8, 6), (8, 10), (10, 7), (11, 0), (11, 10),
]
...
def run_c2_sweep(ablation_type: str):
    import gc
    import sys
    import time
    ...
    circuit_heads = C2_HEADS
    n_players = len(circuit_heads)
    n_total = 2 ** n_players

    out_path = f"/results/c2_{ablation_type}_v2_coalition_values.npz"

    print(f"[{timestamp()}] C2 coalition sweep: {ablation_type} ablation")
    ...
    for idx_pos in tqdm(range(n_total), desc=f"c2/{ablation_type}"):
    ...
    return f"c2/{ablation_type}: {n_total} coalitions complete"
...
def main():
    ...
    print(f"[{datetime.now(timezone.utc).isoformat()}] Launching C2 coalition sweeps: {ablation_types}")
    handles = []
    for abl in ablation_types:
        handles.append(run_c2_sweep.spawn(abl))
    ...
    print(f"\nDownload results:")
    for abl in ablation_types:
        print(f"  modal volume get c2-coalition-sweep c2_{abl}_v2_coalition_values.npz "
