"""Modal wrapper: RTI circuit coalition sweep (zero + mean ablation).

RTI (Repeated Token Identification) is a weaker task than IOI — the circuit
has rho ~0.2 vs IOI's 0.7+, with more distributed interaction structure.
This makes it the natural test case for whether epistatic discovery methods
find circuits that additive methods miss.

Runs the full 2^15 coalition sweep under both zero and mean ablation on
RTI prompts.

Usage:
    cd epistatic-circuits
    modal run --detach scripts/modal_rti_coalition_sweep.py
"""

import modal

app = modal.App("rti-circuit-coalition-sweep")
...
def run_rti_sweep(ablation_type: str):
    ...
    print(f"[{timestamp()}] RTI circuit coalition sweep: {ablation_type} ablation")
    ...
    print(f"[{timestamp()}] {n_prompts} RTI prompts, token shape {all_tokens.shape}")
    ...
    for idx_pos in tqdm(range(n_total), desc=f"rti/{ablation_type}"):
    ...
    return f"rti/{ablation_type}: {n_total} coalitions complete"
...
def main():
    ...
    print(f"[{datetime.now(timezone.utc).isoformat()}] Launching RTI coalition sweeps: {ablation_types}")
    handles = []
    ...
    print(f"\nDownload results:")
    for abl in ablation_types:
        print(f"  modal volume get rti-coalition-sweep rti_{abl}_v2_coalition_values.npz data/")
