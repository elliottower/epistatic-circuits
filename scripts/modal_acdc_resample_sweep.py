"""Modal: Coalition sweep with resample ablation for newly discovered ACDC circuits.

Runs IOI ACDC and RTI ACDC circuits discovered by modal_discover_acdc_ioi_rti.py.
Each has 15 heads -> 2^15 = 32768 coalitions.

Usage:
    cd epistatic-circuits
    modal run --detach scripts/modal_acdc_resample_sweep.py
"""

import modal

app = modal.App("acdc-resample-sweep")

results_volume = modal.Volume.from_name("acdc-resample-sweep", create_if_missing=True)
...
# From ACDC discovery results
CIRCUITS = {
    "ioi_acdc": {
        "task": "ioi",
        "heads": [
            (0, 3), (0, 9), (1, 1), (1, 3), (2, 0), (2, 10), (3, 7),
            (5, 5), (5, 6), (6, 0), (6, 9), (7, 1), (8, 6), (8, 10), (9, 3),
        ],
    },
    "rti_acdc": {
        "task": "rti",
        "heads": [
            (0, 9), (1, 3), (2, 0), (2, 6), (2, 8), (3, 0), (3, 6),
            (3, 7), (4, 6), (5, 4), (5, 7), (6, 9), (7, 9), (8, 6), (8, 10),
        ],
    },
}
...
def run_sweep(circuit_name: str):
    ...
    circuit_heads = circuit_info["heads"]
    ...
    out_path = f"/results/{circuit_name}_resample_coalition_values.npz"
    ...
    print(f"[{ts()}] {circuit_name} resample sweep ({task})")
    ...
    for tmpl in templates:
        for name_a, name_b in cartesian_product(NAMES_A, NAMES_B):
            if task == "ioi":
                clean_text = tmpl.format(S=name_a, IO=name_b)
                corrupted_text = tmpl.format(S=name_b, IO=name_a)
                correct_name, incorrect_name = name_b, name_a
            else:
                clean_text = tmpl.format(D=name_a, C=name_b)
                corrupted_text = tmpl.format(D=name_b, C=name_a)
                correct_name, incorrect_name = name_b, name_a
...
def main():
    ...
    print(f"Spawned {len(handles)} ACDC resample sweeps")
    for h in handles:
        h.get()
    print("All ACDC resample sweeps complete")
