...
app = modal.App("primitive-geometry-v3")
...
out_vol = modal.Volume.from_name("primitive-geometry-results-v3", create_if_missing=True)
...
def run():
    ...
    for (task, hs), arms in sorted(comparable.items(), key=lambda x: x[0]):
        ...
        Path("/out/primitive_geometry_rows_v3.json").write_text(json.dumps(rows, indent=1))
        out_vol.commit()

    # ---- 5. Save per-pair records for P3 --------------------------------------
    note(f"\n{ts()} Saving {len(pair_records)} per-pair records")
    Path("/out/primitive_geometry_pairs_v3.json").write_text(json.dumps(pair_records, indent=1))
    out_vol.commit()
    ...
    Path("/out/primitive_geometry_summary_v3.json").write_text(json.dumps(summary, indent=2))
    Path("/out/primitive_geometry_rows_v3.json").write_text(json.dumps(rows, indent=1))
    Path("/out/primitive_geometry_pairs_v3.json").write_text(json.dumps(pair_records, indent=1))
    out_vol.commit()
