"""Modal: GT coalition sweep with resample (activation patching) ablation.

MIB-style: for each clean prompt, cache corrupted version's hook_z.
Heads OUTSIDE the active coalition get corrupted activations; heads
INSIDE keep clean activations.

Metric: prob_diff = P(years > YY) - P(years <= YY) over 100 year tokens.

Runs 5 circuits: known, acdc, c5_walsh, c6_epistatic, random.
Each has 7 heads -> 2^7 = 128 coalitions.

Usage:
    cd epistatic-circuits
    modal run --detach scripts/modal_gt_resample_sweep.py
"""

import modal

app = modal.App("gt-resample-sweep")
...
CIRCUITS = {
    "known": [(5, 1), (5, 5), (6, 9), (7, 10), (8, 8), (8, 11), (9, 1)],
    "acdc": [(0, 7), (3, 6), (4, 11), (5, 5), (7, 10), (8, 10), (9, 1)],
    "c5_walsh": [(5, 5), (7, 10), (9, 1), (6, 9), (0, 10), (8, 5), (10, 2)],
    "c6_epistatic": [(5, 5), (0, 10), (7, 10), (9, 1), (4, 11), (0, 3), (6, 9)],
    "random": [(0, 6), (2, 2), (2, 4), (2, 11), (4, 9), (5, 2), (5, 10)],
}
...
def run_sweep(circuit_name: str):
    ...
    out_path = f"/results/gt_{circuit_name}_resample_coalition_values.npz"
    ...
    print(f"[{ts()}] GT resample sweep: {circuit_name}")
    ...
    # Set up hooks: mask=1 keeps clean, mask=0 replaces with corrupted
...
def main():
    ...
    print(f"Spawned {len(handles)} sweeps")
    for h in handles:
        h.get()
    print("All GT resample sweeps complete")
